...
class Work {
  ...
  Work(this.title, this.desc, this.words, this.progress, this.lastEdit);
...
class VNote {
  ...
  VNote(this.title, this.desc, this.tag);

  Map<String, dynamic> toJson() => {'title': title, 'desc': desc, 'tag': tag};

  factory VNote.fromJson(Map<String, dynamic> json) {
    return VNote(json['title'] ?? '', json['desc'] ?? '', json['tag'] ?? '设定');
  }
}
...
class OutlineNode {
  ...
  OutlineNode(this.title, this.status, this.line);

  Map<String, dynamic> toJson() => {'title': title, 'status': status, 'line': line};

  factory OutlineNode.fromJson(Map<String, dynamic> json) {
    return OutlineNode(json['title'] ?? '', json['status'] ?? '未写', json['line'] ?? '主线');
  }
}
...
class StoryBlock {
  ...
  StoryBlock.text(this.content) : type = 'text', caption = '';
  StoryBlock.image(this.content, {this.caption = ''}) : type = 'image';

  Map<String, dynamic> toJson() => {'type': type, 'content': content, 'caption': caption};

  factory StoryBlock.fromJson(Map<String, dynamic> json) {
    final type = json['type'] ?? 'text';
    if (type == 'image') {
      return StoryBlock.image(json['content'] ?? '', caption: json['caption'] ?? '');
    }
    return StoryBlock.text(json['content'] ?? '');
  }
}
...
class _MainShellState extends State<MainShell> {
  int index = 0;
  bool loaded = false;
  String activeWorkTitle = '大周：从书院到工业革命';

  List<Work> works = [
    Work('大周：从书院到工业革命', '主角穿越古代，用现代知识建立文明。', 46, 0.10, '刚刚'),
    Work('九州文明设定集', '国家、科技树、人物、势力与时间线。', 32000, 0.38, '昨天 21:10'),
  ];

  List<VNote> settings = [
    VNote('国家设定', '王朝制度、财政、军队、法律与改革阻力。', '国家'),
    VNote('人物档案', '主角、盟友、士族、工匠、商人、反派。', '人物'),
    VNote('科技树', '造纸、水力、冶铁、蒸汽、电气、铁路。', '科技'),
  ];

  List<OutlineNode> outlines = [
    OutlineNode('第一章：穿越到县城书院', '草稿', '主线'),
    OutlineNode('第二章：用造纸术打开第一桶金', '未写', '主线'),
    OutlineNode('第三章：建立水力工坊', '未写', '支线'),
  ];
  ...
  String chapterTitle = '第一章：穿越到县城书院';
  String savedTime = '尚未保存';
  ...
  Future<void> loadAll() async {
    ...
    final worksJson = sp.getString('works');
    final settingsJson = sp.getString('settings');
    final outlinesJson = sp.getString('outlines');
    final blocksJson = sp.getString('blocks');

    setState(() {
      if (worksJson != null) {
        works = (jsonDecode(worksJson) as List).map((e) => Work.fromJson(e)).toList();
      }
      if (settingsJson != null) {
        settings = (jsonDecode(settingsJson) as List).map((e) => VNote.fromJson(e)).toList();
      }
      if (outlinesJson != null) {
        outlines = (jsonDecode(outlinesJson) as List).map((e) => OutlineNode.fromJson(e)).toList();
      }
      if (blocksJson != null) {
        blocks = (jsonDecode(blocksJson) as List).map((e) => StoryBlock.fromJson(e)).toList();
      }
      chapterTitle = sp.getString('chapterTitle') ?? chapterTitle;
      savedTime = sp.getString('savedTime') ?? savedTime;
      activeWorkTitle = sp.getString('activeWorkTitle') ?? activeWorkTitle;
      loaded = true;
    });
  }
  ...
  Future<void> saveAll() async {
    final sp = await SharedPreferences.getInstance();
    await sp.setString('works', jsonEncode(works.map((e) => e.toJson()).toList()));
    await sp.setString('settings', jsonEncode(settings.map((e) => e.toJson()).toList()));
    await sp.setString('outlines', jsonEncode(outlines.map((e) => e.toJson()).toList()));
    await sp.setString('blocks', jsonEncode(blocks.map((e) => e.toJson()).toList()));
    await sp.setString('chapterTitle', chapterTitle);
    await sp.setString('savedTime', savedTime);
    await sp.setString('activeWorkTitle', activeWorkTitle);
  }
  ...
  Future<void> persistAndToast(String text) async {
    await saveAll();
    toast(text);
  }
          ...
          FilledButton(
            onPressed: () async {
              if (title.text.trim().isNotEmpty) {
                setState(() {
                  works.add(Work(title.text.trim(), desc.text.trim(), 0, 0.01, '刚刚'));
                  activeWorkTitle = title.text.trim();
                });
                await persistAndToast('作品已创建并保存');
              }
          ...
          content: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(value: '设定', child: Text('设定')),
                  DropdownMenuItem(value: '人物', child: Text('人物')),
                  DropdownMenuItem(value: '国家', child: Text('国家')),
                  DropdownMenuItem(value: '科技', child: Text('科技')),
                  DropdownMenuItem(value: '势力', child: Text('势力')),
                  DropdownMenuItem(value: '历史', child: Text('历史')),
                ],
                onChanged: (v) => setDialogState(() => tag = v ?? '设定'),
              ),
            ],
          ),
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                if (title.text.trim().isNotEmpty) {
                  setState(() => settings.add(VNote(title.text.trim(), desc.text.trim(), tag)));
                  await persistAndToast('设定已添加并保存');
                }
                if (mounted) Navigator.pop(context);
              },
              child: const Text('保存'),
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: title, decoration: const InputDecoration(labelText: '剧情节点')),
              const SizedBox(height: 10),
              DropdownButtonFormField<String>(
                value: status,
                decoration: const InputDecoration(labelText: '状态'),
                items: const [
                  DropdownMenuItem(value: '未写', child: Text('未写')),
                  DropdownMenuItem(value: '草稿', child: Text('草稿')),
                  DropdownMenuItem(value: '完成', child: Text('完成')),
                ],
                onChanged: (v) => setDialogState(() => status = v ?? '未写'),
              ),
              const SizedBox(height: 10),
              DropdownButtonFormField<String>(
                value: line,
                decoration: const InputDecoration(labelText: '剧情线'),
                items: const [
                  DropdownMenuItem(value: '主线', child: Text('主线')),
                  DropdownMenuItem(value: '支线', child: Text('支线')),
                  DropdownMenuItem(value: '伏笔', child: Text('伏笔')),
                ],
                onChanged: (v) => setDialogState(() => line = v ?? '主线'),
              ),
            ],
          ),
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                if (title.text.trim().isNotEmpty) {
                  setState(() => outlines.add(OutlineNode(title.text.trim(), status, line)));
                  await persistAndToast('大纲已添加并保存');
                }
                if (mounted) Navigator.pop(context);
              },
              child: const Text('添加'),
  ...
  Future<void> saveDraft() async {
    setState(() {
      savedTime = '刚刚';
      if (works.isNotEmpty) {
        works[0].words = wordCount;
        works[0].lastEdit = '刚刚';
      }
    });
    await persistAndToast('草稿已保存到本地');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final pages = [
      ...
      WriterPage(
        ...
        blocks: blocks,
        wordCount: wordCount,
        onChapterTitleChanged: (v) {
          setState(() => chapterTitle = v);
          saveAll();
        },
        onSave: saveDraft,
        onChanged: () {
...
class DashboardPage extends StatelessWidget {
  ...
  final void Function(Work work) onSelectWork;

  const DashboardPage({
    super.key,
    required this.works,
    required this.onAddWork,
    required this.onOpenWriter,
    required this.onSelectWork,
  });

  int get totalWords => works.fold(0, (sum, item) => sum + item.words);
    ...
    return PageScaffold(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: StatCard(title: '总字数', value: '$totalWords')),
              const SizedBox(width: 12),
              const Expanded(child: StatCard(title: '本地存档', value: '已开启')),
            ],
          ),
          const SizedBox(height: 14),
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 6),
                      Text('${work.words} 字 · 最近编辑：${work.lastEdit}', style: const TextStyle(color: Color(0xFF2563EB), fontWeight: FontWeight.bold)),
                    ]),
                  ),
...
class WriterPage extends StatefulWidget {
  ...
  final int wordCount;
  final void Function(String value) onChapterTitleChanged;
...
class _WriterPageState extends State<WriterPage> {
  ...
  void initState() {
    super.initState();
    chapterController = TextEditingController(text: widget.chapterTitle);
  }

  @override
  void didUpdateWidget(covariant WriterPage oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (chapterController.text != widget.chapterTitle) {
      chapterController.text = widget.chapterTitle;
    }
  }
  ...
  Future<void> pickImage() async {
    ...
    setState(() {
      widget.blocks.add(StoryBlock.image(image.path, caption: '图片说明'));
    });
    widget.onChanged();
  }
  ...
  void addTextBlock() {
    setState(() {
      widget.blocks.add(StoryBlock.text(''));
    });
    widget.onChanged();
  }
  ...
  Widget build(BuildContext context) {
    return PageScaffold(
      title: '写作编辑器',
      subtitle: '${widget.activeWorkTitle} · ${widget.wordCount} 字 · 上次保存：${widget.savedTime}',
      action: FilledButton.icon(
...
class _ImageBlockViewState extends State<ImageBlockView> {
  ...
  void initState() {
    super.initState();
    captionController = TextEditingController(text: widget.block.caption);
  }

  @override
  Widget build(BuildContext context) {
    final file = File(widget.block.content);
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Expanded(child: Text('正文图片', style: TextStyle(fontWeight: FontWeight.bold))),
              TextButton.icon(
                onPressed: widget.onDelete,
                icon: const Icon(Icons.delete_outline),
                label: const Text('删除'),
              ...
              child: Row(
                children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(
                      children: [
                        Flexible(child: Text(item.title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16))),
                        const SizedBox(width: 8),
...
class OutlinePage extends StatefulWidget {
  final List<OutlineNode> outlines;
  final VoidCallback onAdd;
...
class _OutlinePageState extends State<OutlinePage> {
  ...
  Color statusColor(String status) {
    if (status == '完成') return const Color(0xFF16A34A);
    if (status == '草稿') return const Color(0xFFF59E0B);
          ...
          children: [
            for (int i = 0; i < widget.outlines.length; i++)
              ListTile(
                ...
                title: Text(widget.outlines[i].title),
                subtitle: Text('${widget.outlines[i].line} · 点击可进入章节规划'),
                trailing: Text(
                  widget.outlines[i].status,
                  style: TextStyle(
                    color: statusColor(widget.outlines[i].status),
                    fontWeight: FontWeight.bold,
                  ),
                ),
                onTap: () => showInfo(context, '大纲节点', '${widget.outlines[i].line} · ${widget.outlines[i].status}\n\n${widget.outlines[i].title}'),
              ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return AppCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF2563EB))),
      ]),
    );
  }
}
...
class TagChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text, style: const TextStyle(fontSize: 12, color: Color(0xFF2563EB), fontWeight: FontWeight.bold)),
    );
  }
}
